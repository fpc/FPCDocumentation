...
The Free Pascal compiler initially emulated most of \tp and lated on
...
The \delphi reserved words are the same as the \tp ones, plus the
following keywords:
\begin{multicols}{4}
\begin{verbatim}
as
...
On top of the \tp and \delphi reserved words, \fpc also considers
...
Identifiers consist of between 1 and 255 significant characters (letters, digits and
...
jumped from another location with a \var{Goto} statement. Labels can 
be digit sequences or identifiers.\keywordlink{label}
...
The string consists of standard, 8-bit ASCII characters. The \var{control
string} can be used to specify characters which cannot be typed on a
keyboard, such as \var{\#27} for the escape character. 
...
\begin{verbatim}
  ...
  ''
  'a'
  'A tabulator character: '#8' is easy to embed'
\end{verbatim}
...
\end{verbatim}
on a Mac-like operating system.
...
run-time. Their definition is quite simple:

\input{syntax/tconst.syn}

This is an old concept from Turbo Pascal, which has been replaced with 
support for initialized variables: For a detailed description, see 
\sees{initializedvars}.

Support for typed constants is controlled by the \var{\{\$J\}} directive: 
it can be switched off, but is on by default (for Turbo Pascal compatibility).
Initialized variables are always allowed.
...
\var{Delphi} or \var{Objfpc} modes.
...
...
 Inc (p);
\end{verbatim}
will increase \var{P} with 4, because 4 is the size of a longint.
...
\var{exclude} & exclude an element from the set\\ \hline
\end{FPCltable}
...
\subsection{overload}
\label{se:overload}\index{overload}\index{Modifiers!overload}
...
\subsection{public}
\label{se:public}\index{Modifiers!public}\index{public}
...
all \var{raise} statements, since then the message field of the exception 
object can be used.
