...
Note that they have the \var{class} keyword prefixed.
...
must always contain two parameters, with the exception of the unary minus,
where only one parameter is needed. The result type of the comparison
operator must be \var{Boolean}.

For Delphi compatibility, the symbolic operator names can also be replaced by a plain-text
name:
...
\end{FPCltable}
\begin{remark}
When compiling in \var{Delphi} mode the names of the operators {\em must} be used,
not the symbols.
\end{remark}
