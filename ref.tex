...
Whenever the identifier is used, then a warning will be displayed,
corresponding to the hint that was specified.
\begin{description}
...
\item If the switch is on \var{\{\$H+\}}, and there is a maximum length
...
\begin{remark}
Short strings have a maxmimum length of 255 characters: when specifying a
...
Turbo Pascal code relies on this, and it is implemented similarly in Free
...
retrieved with the \var{Length} function or set with the \var{SetLength}
function. Both functions work on shortstrings as well, and should be used
for code that must compile in both Object Pascal as old Turbo Pascal mode.
\end{remark}
...
\begin{remark} 
The null-termination does not mean that null characters (char(0) or \#0) cannot
be used: the null-termination is not used internally, but is there for
...
\begin{verbatim}
 L:=ord(S[0]);
\end{verbatim}
which was valid for Turbo Pascal shortstrings, is no longer correct for
...
require an instance: The \var{Self} identifier refers to the class pointer 
in such methods. 
...
\end{verbatim}
As an extention to Delphi syntax, the ObjFPC mode also supports a special
...
\end{verbatim}
It can be seen as the equivalent of the C \var{return} instruction.
...
\begin{remark}
Note that \var{inline} is a hint for the compiler. This does {\em not}
automatically mean that all calls are inlined; sometimes the compiler
may decide that a function cannot be inlined, or that a particular call
