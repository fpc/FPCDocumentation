...
in a record and on field's location in the record.
...
\var{\{\$PACKRECORDS N\}} setting is
  ...
  \item \var{RESET} or \var{DEFAULT}: the resultinf required alignment
  is target dependent
...
The size of a record is equal to the sum of record's last field's offset
and this field's size, rounded up to a multiple of the record's required
alignment. The record's required alignment is calculated as followed:
\begin{itemize}
  ...
  record
  \item If the currently \var{\{\$ALIGN N\}} or \var{\{\$PACKRECORDS
