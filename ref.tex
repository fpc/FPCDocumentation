...
\item \var{Char},
\item \var{String}
\item Arrays of any of the above types.
\end{itemize}
...
for..in loop is used in case one wants to calculate something a fixed number of times
...
\begin{itemize}
\item Whether then class in the enumerable expression implements a method
\var{GetEnumerator}
...
\end{verbatim}
Note that the compiler enforces type safety: Declaring D as an integer will
...
signature as the \var{IEnumerate} approach above. The following code will
