...
\begin{remark}
In accordance with ANSI pascal specifications, \fpc first calculates the
upper bound, and only then initializes the counter variable. This behaviour
was changed, old versions of \fpc did things in reverse order.
\end{remark}
...
\begin{verbatim}
  ...
  TestIt ([AClass]);
\end{verbatim}
Of course it is also possible to pass elements of different types:
\begin{verbatim}
  S:='Ansistring 1';
  T:='AnsiString 2';
  Testit ([1,S]);
  Testit (['A',TRUE]);
  Testit (['String',1,True]);
  Testit ([S,Nil]);
  TestIt ([AClass]);
\end{verbatim}
