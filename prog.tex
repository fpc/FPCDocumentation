...
Therefore, it is set to \var{OFF} by default.
...
Reversely, in order to use the \var{Published} keyword in a class, it must
be a descendent of a class that was compiled in the \var{\$M+} state, or be
compiled in the \var{\$M+}

By default, no Run-Time Type Information is generated. The \var{TPersistent}
object that is present in the \file{classes} unit (part of the RTL) is generated in
...
The \var{\{\$Z\}} form takes a boolean argument, where \var{ON} is equivalent
to \var{\{\$Z4\}} and \var{OFF} is equivalent to \var{\{\$Z1\}}.
...
\label{se:Push}
The \var{\$POP} directive saves the values of all local compiler
...
Its name is \file{prt0.o}, and the
...
Its main function is to save the environment and command line arguments and
