...
Pascal, the \var{\{\$Q \}} directive can be used on an expression-level.
...
Pascal, the \var{\{\$R \}} directive can be used on an expression-level.
...
FPU values. If set to ON, the compiler inserts a \var{FWAIT} opcode after
...
The LIBSUFFIX directive has a similar function as the \var{\{\$EXTENSION \}} and
\var{\{\$LIBPREFIX \}} compiler directives. directive: it sets the suffix of the 
...
The \var{\{\$MINSTACKSIZE\}} sets the maximum stack size for an executable
...
\item[ADVANCEDRECORDS] allow the use of advanced records (i.e. records with 
methods)
\end{description}
...
that is, the compiler will try to pass as much parameters as possible by 
