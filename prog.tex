...
Be default, the compiler always uses shortcut evaluation, i.e. the evaluation 
of a boolean expression is stopped once the result of the total exression is 
known with certainty. The \var{\{\$B \}} switch can be used to change this
behaviour: if it's argument is \var{ON}, then the compiler will always
evaluate all terms in the expression. If it is \var{OFF} (the default) then
the compiler will only evaluate as many terms as is necessary to determine
the outcome.
...
\var{\{\$H\}}: a \var{\{\$MODE DELPHI\}} statement implies a
\var{\{\$H+\}} statement. This behaviour has changed, in older \fpc
versions this was not so.
...
string. The possible strings are the same as can be specified on the
command-line and are documented in the user's guide.
\end{description}
Example:
\begin{verbatim}
{$OPTIMIZATION ON}
...
This switch is also activated by the \var{-Oxxx} command-line switch.
...
{$info At least this is version 2.0}
{$else}
...
FPC\_OBJFPC & \fpc is in OBJFPC mode, either using compiler switch -S2 or \\
...
CPUX86\_64 & \fpc target is a AMD64 or Intel 64bit processor.\\
