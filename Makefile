...
ifndef OLDGRAPH
GRAPHDIR=$(FPCSRCDIR)/packages/base/graph
else
...
FCLPROCESS= --descr=process.xml --input="$(FCLBASEINC)/process.pp"
FCLDBUGINTF= --descr=dbugintf.xml --input="$(FCLBASEINC)/dbugintf.pp" 
FCLCONTNRS= --descr=contnrs.xml --input="$(FCLBASEINC)/contnrs.pp"
FCLZSTREAM= --descr=zstream.xml --input="$(FCLBASEINC)/zstream.pp"
...
FCLEZCGI= --descr=pooledmm.xml --input="$(FCLBASEINC)/pooledmm.pp"
