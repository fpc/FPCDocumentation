...
For Windows, there is a Windows installer, \file{setup.exe}, this is a normal 
...
\end{verbatim}
Where \var{X.Y.Z} is the version number of the \file{.rpm} file, and \var{ARCH} is one of the supported architectures (i586, x86\_64 etc.).
...
replaced by all directories found on that location. For example, given the
directories
\begin{verbatim}
...
It will do this first with the extension \file{.pp} and then with the
extension \file{.pas}.
...
Not all options mus be given on the compiler commandline. The compiler
can use a configuration file wich can contain the same options as on the
...
file \file{ppc386.cfg}. This file is still searched, but it's usage 
...
\fpc can handle long filenames under on all platforms, except DOS; 
On windows, it will use support for long filenames if it is available
(which is not always the case on older windowses).
...
program, leaving out any unnecessary code. The compiler will turn on
smartlinking with the \seeo{XX} switch. This technique is described in
...
category. Also, \seec{commandlineoptions} for a listing as generated by 
...
\item \var{i} : Tells the compiler to issue informational messages.
...
\item \var{v} write a file \file{fpcdebug.txt} with lots of debugging info.
Mainly for the compiler developers.
\item \var{p} write a file \file{tree.log} with the parse tree.
Mainly for the compiler developers.
\end{itemize}
\end{description}
...
directory \file{xxx} instead of the current directory. If this option
is followed by a  \var{-o} option (\seeo{o}), and this option contains a path 
component, then it will override the \var{-FE} setting.
\olabel{FE}
...
possible directory names found at that location.
\olabel{Fu}
...
\item [-oxxx] \olabel{o}Tells the compiler to use \var{xxx} as the name of the output
file (executable). Only with programs. The output filename can contain a
...
is assumed). Instead of a number, one of \var{n}, \var{h} or \var{w} can also be specified, in that case the compiler will consider notes, hints or warnings as errors and stop when one is encountered.
...
\fpc. (however, the \seeo{M} option solves this.)
...
\item There are more reserved words. \var{PROTECTED, PUBLIC, PUBLISHED, TRY, FINALLY, EXCEPT, RAISE}
...
they are in Delphi. Using the \var{-Mtp} switch will solve this problem if
...
\item The \var{TextRec} structure (for internal description of files) is not
binary compatible to TP or Delphi.
...
\fpc as of version 2.0. The reason for this incompatibility is below:
...
\item Enumerateds are of different size than in Delphi.
...
bytes of a word or integer may not be on the same location as on an Intel
...
on an intel 32-bit processor, but not necesarily on other processors.
...
\item [231 Widestring error]
Some widestring operations, like conversion to and from ansistrings, conversion
to upper and lower case and comparison are handled by a special unit, the
widestring manager. This runtime error occurs if the widestring manager could
