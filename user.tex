...
\item \var{v} : Write copious debugging information to file fpcdebug.txt.
...
\olabel{FE}
\item [-Fixxx] Add \var{xxx} to the include file search path.
\olabel{Fi}
\item [-Flxxx] Add \var{xxx} to the library search path. (This is also 
...
messages. This will override the compiler's built-in default messages.
\olabel{Fr}
...
\item [-Ct] \olabel{Ct} Fenerate stack checking code. If your program
...
\item[objfpc] Switch on some Delphi 2 extensions. This is different from 
...
\end{verbatim}
if the compiler version is \var{0.99.12a} and the target OS is \linux{}.
...
\item In Turbo Pascal, parameter lists of previously deﬁned
functions and procedures did not have to match exactly. In
Free Pascal, they must. The reason for this is the function overloading mechanism of
