...
% Settings
%
\makeindex[name=msgnr,title=Message index by number,columns=4,options=-s ./index]
\makeindex[name=msgtxt,title=Message index by text,options=-s ./index]
%
% Start of document.
%
\begin{document}
...
The supported (Object) Pascal Language constructs are described in the \refref,
and the available routines (units) are described in the \unitsref and FCL Unit reference guides.
...
programs with the Free Pascal compiler. 
...
described in full in the \progref.
...
\item[-l]  Print a line stating which program this is and its version.
\olabel{l}
...
\item \var{0} : No messages. This is useful for overriding the default setting in the configuration file.
...
\item \var{v} : Write copious debugging information to file \file{fpcdebug.txt}.
...
that all pointer accesses are within the heap. (note that you cannot use
this for externally obtained pointers).
...
\item [-gv] Emit info for Valgrind. {\em Note:} this will include the \var{cmem}
...
boundary. See the \progref for a description of the possible
values.
...
\item[G] Generate a GUI application (+) or a console application (-). For
Windows-like operating systems a graphical application does not have
stdin/stdout/stderror file descriptors. If you create a console application, 
then these file descriptors are opened when the program is started. Note that
in a console application you can still create windows as in a graphical application.
...
\item [-St] \olabel{St} Allow the \var{static} keyword in objects. This
flag is obsolete since \fpc 2.6.0.
...
The installer of the FPC compiler generates a default configuration file
in one of the places where the compiler looks for it (see also
 \ref{searchconfig} on page \pageref{searchconfig}). 
It sets the unit path to wherever the installer  put the units, and sets some minor syntax options. 
...
This is because their purpose was limited to a 16-bit environment where
calls were either far or near. This distinction does not exists in 32/64 bit
environments.
...
\item Turbo Pascal's 'overlay' unit is not available. This unit had it's
purpose in a 16-bit MS-DOS environment, but in modern environments this is
no longer relevant.
...
\subsection{Options}
\file{h2paspp} has a small number of options to control its behavior:
\begin{description}
...
files will have an extension \file{.ppl}; this can be changed with the
...
You should not include this unit directly, instead use the \var{-gh} command-line switch.
...
For more information, refer to the \var{gdb} of \userref, or use the
...
The unit that does this is called \file{heaptrc}.
Do not include this unit in your uses clause directly.
Instead, supply the \var{-gh} switch to the compiler, and it will include
...
generated by the compiler on linux 64-bit (the list may vary depending on
the platform):
\input{comphelp.inc}
...
by using the \var{cwstring} unit which uses libc, or the \var{fpwidestring} unit
...
\begin{verbatim}
http://downloads.freepascal.org/fpc/snapshot/trunk/
\end{verbatim}
and
\begin{verbatim}
http://downloads.freepascal.org/fpc/snapshot/fixes/
\end{verbatim}
respectively.
...
\chapter{Terminal emulator configuration examples}
\label{ch:TerminalConfigExample}

The provided configuration files are designed to replicate the experience of
using the Turbo Pascal IDE. This includes a DOS-like color scheme and cursor
style (underscore). \key{Ctrl-Ins}, \key{Shift-Ins} should work as expected.
Copying and pasting within the terminal are remapped to \key{Ctrl-Shift-C} and
\key{Ctrl-Shift-V}, respectively.

\section{XTerm terminal configuration file}

The following content should be placed in the \file{\~{}/.Xresources} file.
This file is loaded automatically upon logging in.
...
\section{Kitty terminal configuration file}

The following content should be appended to \file{\~{}/.config/kitty/kitty.conf}.
This configuration file is loaded when the Kitty terminal starts.
