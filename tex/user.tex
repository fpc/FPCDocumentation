...
Provided configuration files are designed to match experience of using Turbo
Pascal IDE. DOS like color schema, cursor style - underscore. \key{Ctrl-Ins},
\key{Shift-Ins} must work. Copy and paste in terminal itself are remapped to
\key{Ctrl-Shift-C} and \key{Ctrl-Shift-V} respectively.

\section{XTerm terminal configuration file}

Following content should be placed 
in \file{\~{}/.Xresources} file. File is loaded on logging in.
...
Following content should be appended to \file{\~{}/.config/kitty/kitty.conf} file.
Configuration file is loaded on startup of Kitty terminal.
