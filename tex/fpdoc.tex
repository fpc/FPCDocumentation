...
\latex{%
  \ifpdf
      \hypersetup{
           ...
           pdftitle={FPDoc Reference Manual},
           pdfsubject={FPDoc reference manual},
...
The \tag{imports} tag below the \tag{package} tag contains one
\tag{import} tag for each content file to import file belonging to the package.
No other tags should appear below the \tag{imports} tag.
...
The \tag{import} tag describes one import file used in the
documentation for the package. It has one attribute \attr{file}, containing
the filename of the documentation file. This attribute is mandatory. All
other attributes are ignored.
...
% Tag i
\subsection{\tag{file} : Format filename}
\label{tag:file}
The \tag{file} tag will cause its contents as a filename. It can
occur mixed with any text. It is similar to the \seetag{var} tag.
...
\end{verbatim}
will be formatted as:\\
The file \file{me.exe<} is searched in the PATH.

See also: \seetag{b},  \seetag{i}; \seetag{u}, \seetag{var} 
...
See also: \seetag{b}, \seetag{u}, \seetag{i}, \seetag{code},  \seetag{file}
