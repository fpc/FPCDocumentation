...
%
\subsection{Compiling first program}
...
\emph{"Hello world!"} program will be great choice. Open \menu{File|New} and
enter program: 
\begin{verbatim}
begin
  writeln('Hello world!');
...
\end{verbatim}
Save program with \menu{File|Save as}. Program has to be saved before it can be
compiled.

Next step is to test your program. Select from menu \menu{Run|Run} or press
\key{Ctrl-F9} (\sees{running}). This command compiles your program if not already
or if it is modified and runs it. With success you will see \emph{"Hello world!"}
message printed on screen. Press \key{Alt-F5} to see \emph{User screen} if
program have returned to the IDE.

Compilation might fail. That is normal part of development. Address issue
identified by error message. Keep compiling until success and program can be run.
...
\begin{remark}
The IDE contains compiler within itself and it does not need to locate command
line compiler. Location of used units have to be supplied. Unit \emph{system}
is used by every program and unit. IDE do  make an educated guess and try to
preconfigure location of units shipped with \fpc.
\end{remark}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
