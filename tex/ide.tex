...
\item For better experience in work with IDE it is recommended to setup font,
font size, colors and other aspects of console/terminal to your liking. XTerm
and Kitty terminal example configuration files are listed in appendix {\ref{ch:TerminalConfigExample}}.
\end{itemize}
\end{remark}
%
...
There should be no whitespace between the file name and the \var{-C}. Extension
of \file{filename} is ignored and files \file{filename.cfg}, \file{filename.dsk} and
\file{filename.ini} are used. More information on the configuration files can be
found in \sees{ideconfigfiles}.
\item [-R] Or \var{-R+}. After starting the IDE, change automatically to the
directory which was active when the IDE exited the last time. \var{-R-} to keep
statup directory as active directory. Overrides configuration option
\emph{Return to last directory} (\sees{prefdesktop}).
...
Configuration files are essential for IDE to work. If none is found then they
will be created, thus IDE expects to have write permission in directory where
it is started from.

Configuration consists of 3 files:
\begin{itemize}
\item \file{fp.cfg} - save compiler related configuration and this file is
passed to compiler as command line parameter @fp.cfg
\item \file{fp.dsk} - save data that should be preserved across IDE sessions.
Chose what to save in desktop file in \menu{Options|Environment|Desktop...}
\item \file{fp.ini} - configuration of IDE itself.
\end{itemize}
Whenever you start IDE in directory first time, there are 4 scenarios for IDE
configuration files.
\begin{enumerate}
\item Use default configuration files. IDE creates file \file{fp.dir}.
\menu{Options|Environment|Preferences} \emph{Desktop file: Config file
directory}.
\item Use default configuration files. Create desktop file in current
directory. IDE creates file \file{fp.dir}. \menu{Options|Environment|Preferences}
\emph{Desktop file: Current directory}.
\item Copy default configuration files. Save in current directory.
\item Create new configuration files. Save in current directory.
\end{enumerate}
...
\begin{enumerate}
\item \file{\~{}/.fp}
\item Directory of IDE executable.
...
\end{enumerate}
\end{itemize}
On sequential IDE start up in same directory ether \file{fp.cfg}, \file{fp.dsk}
and \file{fp.ini} from current directory will be used or default configuration
files will be used if file \file{fp.dir} exists.
%
% The IDE screen
%
...
the selected file in a new editor window. In a file list by entering beginning
of file name respective file will be searched and selected. To search for a
directory enter first letter as capital letter.
...
\item[Comment] Insert line comment in front of every selected line.
\item[Uncomment] Remove line comment from beginning of every selected line.
...
\item[List] (\key{Alt-0})
Shows the list of opened windows. From there a window can be activated,
closed, shown and hidden. List is searchable by file name.
...
\item[Desktop]
Configure some aspects of desktop:
\begin{description}
\item[Show clock] In right upper corner of desktop clock will be shown.
\item[Show heap monitor] Show used memory in kilobytes in right lower corner of desktop.
\end{description}
\item[Auto save]
...
\item[Return to last directory] When checked, on startup current directory
will be set to what it was upon exit the IDE last time. Off by default.
\end{description}
