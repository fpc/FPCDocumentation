...
\begin{remark}
\begin{itemize}
\item Under \windows, it is possible to switch between windowed mode and
full screen mode by pressing \key{Alt-Enter}.
\item For a better user experience, it is recommended to customize the font,
font size, colors, and other console/terminal settings to your preferences.
Example configuration files for XTerm and Kitty terminals are listed in
Appendix {\ref{ch:TerminalConfigExample}}.
\end{itemize}
\end{remark}
%
...
There should be no whitespace between the file name and the \var{-C}. The
extension of the \file{filename} is ignored, and files named \file{filename.cfg},
\file{filename.dsk} and \file{filename.ini} are used. More information on the
configuration files can be found in \sees{ideconfigfiles}.
\item [-R] Or \var{-R+}. After starting the IDE, change automatically to the
directory which was active when the IDE exited the last time. \var{-R-} keeps
the startup directory as the active directory. This overrides the configuration
option \emph{Return to last directory} (\sees{prefdesktop}).
...
%
\subsection{IDE configuration files}
\label{se:ideconfigfiles}
Configuration files are essential for the IDE to function properly. If none are
found, they will be created automatically. Therefore, the IDE expects to have
write permission in the directory from which it is started.

The configuration consists of three files:
\begin{itemize}
\item \file{fp.cfg} - Stores compiler related settings. This file is
passed to the compiler as command line parameter: \var{@fp.cfg}.
\item \file{fp.dsk} - Stores data that should be preserved across IDE sessions.
You can choose what to save in this desktop file via \menu{Options|Environment|Desktop...}.
\item \file{fp.ini} - Contains the IDE's own configuration settings.
\end{itemize}
When you start the IDE in a directory for the first time, there are four
possible scenarios for the configuration files:
\begin{enumerate}
\item Use default configuration files. The IDE creates a file named \file{fp.dir}.
\menu{Options|Environment|Preferences} \emph{Desktop file: Config file
directory} is selected.
\item Use default configuration files. The IDE creates the desktop file and file
named \file{fp.dir} in the current directory. \menu{Options|Environment|Preferences}
\emph{Desktop file: Current directory} is selected.
\item Copy default configuration files and save them in the current directory.
\item Create new configuration files and save them in the current directory.
\end{enumerate}
...
\begin{enumerate}
\item \file{\~{}/.fp}
\item Directory of the IDE executable.
\item Current directory.
\end{enumerate}
\item \windows:
\begin{enumerate}
\item \file{APPDATA$\backslash$fp}\footnote{\file{APPDATA} means directory
in environment variable \file{APPDATA}.}
\item Directory of the IDE executable.
\item Current directory.
\end{enumerate}
...
\end{enumerate}
\end{itemize}
On each sequential startup of the IDE in the same directory, ether \file{fp.cfg},
\file{fp.dsk} and \file{fp.ini} from the current directory will be used or
default configuration files will be used if the \file{fp.dir} file exists.
%
% The IDE screen
%
...
The menu provides access to all of the IDE's functionality.
...
%
\subsection{Compiling your first program}
To ensure that IDE is working and is setup properly, it is recommended that your
first program is as simple as possible, but with useful feedback.
A simple \emph{"Hello world!"} program is a great choice. Open \menu{File|New} and
...
\end{verbatim}
Save the program with \menu{File|Save as}. The program has to be saved before it can be
compiled.

The next step is to test your program. Select the menu item \menu{Run|Run} or press
\key{Ctrl-F9} (\sees{running}). This command compiles your program if it was
not already compiled or if it was modified since the last compile, and runs it. 
In case of success you will see a \emph{"Hello world!"}
message printed on the screen. Press \key{Alt-F5} to see the \emph{User screen}
when the program has returned to the IDE.

Compilation can fail. Failing is a normal part of development. In such a
case, address the issue identified by the error message. 
Keep compiling until the program compiles successfully and the program can be run.

Here is how to resolve error: \emph{Fatal: Can't find unit system used by Program}.

Open \menu{Options|Directories…} and in tab \emph{Units} enter valid path to
location of \file{system.ppu} (\sees{directorydialog}). If you have entered path,
but error persists then open \menu{Options|Compiler…} in tab \emph{Verbose}
check \emph{Used, tried info} to get more information during compilation for
further troubleshooting. 

\begin{remark}
The IDE encorporates the compiler and it does not need to locate the command
line compiler. The location of used units has to be supplied. The \emph{system}
unit is used implicitly by every program and unit. The IDE does make an educated 
guess and tries to preconfigure the location of units shipped with \fpc.
\end{remark}
...
and the cyan-colored rectangle will scroll the window's content
...
window in the list of windows, while \menu{Window|Previous} will select
the previous window.
...
functionality of the IDE. The menu is always available, except when a dialog is opened. If a
...
the selected file in a new editor window. In the file list, entering the
beginning of a file name will search for and select the respective file. To
search for a directory, enter the first letter with a capital letter.
...
\item[Comment] Adds a line comment at the beginning of each selected line.
\item[Uncomment] Removes the line comment from the beginning of each selected line.
...
\item[List] (\key{Alt-0})
Shows a list of opened windows. From there a window can be activated,
closed, shown and hidden. The list is searchable by file name.
...
%
\subsection{Clipboard, copying and pasting}
The IDE has its own clipboard, which is independent of the operating system's
clipboard. The {\em Cut}, {\em copy} and {\em paste} commands operate with
this internal clipboard. Selected text in the {\em Clipboard window}
(accessible via \menu{Edit|Clipboard}) can be pasted into other parts of
the IDE that accept text input.

To access the OS clipboard, use commands from the menu
\menu{Edit|Copy from Windows} and \menu{Edit|Paste to Windows}. If interaction
with the OS clipboard is not supported, these menu options will not be visible.

The default shortcut keys for {\em cut}, {\em copy} and {\em paste} are
\key{Shift-Del}, \key{Ctrl-Ins} and \key{Shift-Ins}, respectively. On \linux,
the \key{Ctrl-Ins} and \key{Shift-Ins} may not be available to the IDE without
special terminal configuration (see appendix \ref{ch:TerminalConfigExample}).

It is possible to activate shortcut keys \key{Ctrl-X}, \key{Ctrl-C} and
\key{Ctrl-V} for {\em cut}, {\em copy} and {\em paste} commands
(\sees{prefmouse}). In this case, the default functionality of these shortcuts
will be overridden.
%
% Bookmarks
%
...
three predefined tools for programmers: an ASCII table, a grep tool
...
\end{description}
\begin{remark}
For the search path an asterisk (\var{*}) as the last directory will include all
subdirectories one level deep.

Path substitution variables such as \var{\$fpctarget }and \var{\$fpcversion }can
be used (\sees{VariableSubstInPath}).
\end{remark}
%
...
\begin{description}
\item[Dos (go32v1)] This switch will disappear in time as this target is no
...
The menu \menu{Options|Compiler} allow the setting of options that affect the
compilers behavior. When this menu item is chosen, a dialog pops up that
...
\item[Desktop]
Configure certain aspects of the desktop:
\begin{description}
\item[Show clock] Displays a clock in the upper right corner of the desktop.
\item[Show heap monitor] Displays used memory in kilobytes in the lower right
...
\item[Return to last directory]
When checked, othe current directory will be set to the directory last
used upon exiting the IDE. Off by default.
\end{description}
...
the overwrite cursor is a block. This option reverses that behavior. Off by
default.
...
option disables that behavior; i.e. any trailing spaces are also saved
...
Beginning of Line & \key{Home} & \key{Ctrl-Q-S} \\
...
Goto line number & \key{CTRL-B} & \key{Ctrl-Q-G} \\
\end{FPCltable}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Delete till end of line & \key{Ctrl-Q-Y} & \\
Delete till beginning of line & \key{Ctrl-Q-H} & \\
...
Uppercase word & \key{Ctrl-K-F} & \\
Lowercase word & \key{Ctrl-K-E} & \\
Toggle case & \key{Ctrl-O-U} & \\
\end{FPCltable}
...
Remove selection & \key{Ctrl-K-Y} & \key{Ctrl-Del}\\
Extend selection one char to the left & \key{Shift-Arrow left} & \key{Ctrl-Shift-A}\\
Extend selection one char to the right & \key{Shift-Arrow right} & \key{Ctrl-Shift-F} \\
...
Open File at cursor & \key{Ctrl-Enter} & \key{Ctrl-O-A}\\
\end{FPCltable}
