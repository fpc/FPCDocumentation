...
For the \var{MyRec} record above, the following would initialize all fields:
\begin{verbatim}
var
  R : MyRec = (X : 1; Y: 2; Z: 3);
\end{verbatim}
...
\end{verbatim}
In ObjFPC mode, it is not allowed to skipfields; you can specifiy field
values up to a specific field. Thus, the following will compile in Delphi
mode, but not in ObjFPC mode:
\begin{verbatim}
program Project1;
type
  TPoint = record
...
\end{verbatim}
The following will compile in both modes:
\begin{verbatim}
program Project1;
type
  TPoint = record
