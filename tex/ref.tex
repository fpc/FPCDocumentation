...
\end{FPCltable}
In addition to the simple \var{Boolean} type, the additional  \var{Boolean16}, \var{Boolean32} and \var{Boolean64}
...
The only difference with the \var{Boolean16/32/64} types is in what values
...
assumptions about the template type \var{T}. Consider the following unit:
...
\end{verbatim}
The compiler will throw an error because when it compiles the generic
definition, it cannot verify that
\begin{verbatim}
   lst:=lst^.next;
\end{verbatim}
is correct. \var{lst} is of type \var{T}, but the compiler does not (yet)
know what \var{T} is, and hence cannot know it has a field \var{next}.
...
\end{verbatim}
Here, the compiler knows that \var{lst} is at least of type TListEl, and
hence contains members \var{Prev} and \var{Next}.
