...
about the various modes can be found in the \userref and the \progref.
...
For some syntactical elements you will find no definition in the various diagrams. 
This is because they are defined implicitly in the pascal parser: the tokens of the pascal language.

The following elements are defined implicitly:
\begin{description}
\item[string] This is a literal string. 
\item[number] This can be an integer number or a floating point number.
\item[identifier] This is basically a name the programmer has given to an item he wishes to use in his code.
This item can be a type, a variable, a constant or even the name of a procedure or unit.
\end{description}
...
about the various modes can be found in the \userref and \progref.
...
Moreover, a specifier can combine different forms of left and right parentheses.
...
\end{verbatim}
This is an older form of comment notation, and nowadays the  \var{\{} and \var{\}}
characters are the standard notation:
\begin{verbatim}
{ My beautiful function returns an interesting result }
Function Beautiful : Integer;
...
such constructs, regardless of the mode and feature switches.

\begin{remark}
For compatibility reasons, nested comments are disabled by default in some modes (e.g. \var{TP} and \var{Delphi}).
You can change this using the \var{MODESWITCH} directive with the value \var{NESTEDCOMMENTS}.
\end{remark}
...
Dotted identifiers are an identifier which can contain a dot in the name. 
Fully qualified identifiers are dotted identifiers with the implied meaning that the first part is a unit name, and the second part is the name of an identifier in that unit. 
Both the unit identifier and the latter can of course also be dotted identifiers in themselves.
...
well as the invocation of the procedure \var{do}.
\end{remark}

\section{Hint directives}
\index{Hint directives}\index{Directives!Hint}\label{se:hintdirectives}
...
\index{String}\index{Constants!String}\index{Tokens!Strings}\index{String literal}
A character string or string literal (string for short) is a sequence of zero or more
characters (byte or word sized), enclosed in single quotes, and on a single
...
As with all identifiers, constants can be accompagnied by hint directives
(\sees{hintdirectives}):
\begin{verbatim}
const
  MyPi = 3.14 deprecated;
\end{verbatim}
...
In the case of an expression, the expression must be computable at runtime, 
just as for a normal constant, and the type of the expression must be assignment 
compatible to the type of the constant.
...
A syntax diagram has no notion of the type of an identifier or expression.
However, sometimes it is useful (or necessary) to know the type of an identifier.
To make the syntax diagrams more clear, the type of an identifier has been added as a 
prefix to the name. The following is a list of such typed identifiers:
\input{syntax/typedident.syn}
from a perspective of language syntax, they can all be replaced by \var{fully-qualified-identifier}
...
check error if range checking is enabled.
\item Integer ordinal types are mutually assignment compatible: there is no need to
typecast when assigning an ordinal value of one (e.g. smallint) to another
(e.g. byte). If you assign a value to a variable of an ordinal type, 
and that value is actually out of the range of valid values for that type, 
this will not result in a run-time error, although it may be a logical error. 
Only when range checking is enabled will such an assignment result in a range
check error.
\end{enumerate}

The fact that ordinals are mutually assignment compatible means that the 
following program will work without errors when compiled without range checking:
\begin{verbatim}
var
   b: byte;
   si: integer;
...
\end{verbatim}
Only when you compile with range checking by specifying \var{-Cr} on commandline, or
adding \var{\{\$R+\}} to the source code, will the compiler insert code to
check whether the assigned value is valid for the target ordinal type.

In the last example, a typecast is used, forcing the right-hand side of the
assignment to the same type as the left-hand side. In that case, no range
checks will be inserted, even when range checking is enabled.
...
\index{Types!Integer}
\keywordlink{Integer}
\keywordlink{Shortint}
\keywordlink{SmallInt}
\keywordlink{Longint}
\keywordlink{Longword}
\keywordlink{Int64}
\keywordlink{Byte}
\keywordlink{Word}
\keywordlink{Cardinal}
\keywordlink{QWord}
\keywordlink{ByteBool}
\keywordlink{WordBool}
\keywordlink{LongBool}
\keywordlink{QWordBool}
A list of predefined integer types is presented in \seet{integerstyp}.
...
\fpc are listed in \seet{integersranges}.

\begin{FPCltable}{lcr}{Predefined integer ranges}{integersranges}
...
The \var{integer} type is an alias to the smallint type in the default\keywordlink{integer}
...
in \seet{integerconsttypemapping} is used.
...
\begin{remark}
In newer Delphi versions, the \var{longint} type is platform and CPU dependent.
...
\fpc supports the \var{Boolean} type, with its two predefined possible
...
\end{FPCltable}
In addition to the simple \var{Boolean} type, the additional  \var{Boolean8}, \var{Boolean16}, \var{Boolean32} and \var{Boolean64}
types exist. These are in fact integer types, which are assignment-compatible with the simple boolean
type. As an integer, the values for \var{True} and \var{False} are 1 and 0.
...
\var{ByteBool}, \var{WordBool}, \var{LongBool} and \var{QWordBool} types.
These are of type \var{Byte}, \var{Word}, \var{Longint} or \var{Int64}, but are
again assignment compatible with a \var{Boolean}.
The only difference with the \var{Boolean8/16/32/64} types is in what values
are considered true or false: The value \var{False} is equivalent to 0 (zero)
and any nonzero value is considered \var{True} when converting to a boolean value.
A boolean value of \var{True} is converted to \var{Not(0)} in case it is assigned to a
variable of type \var{ByteBool}, \var{WordBool}, \var{LongBool} or
\var{QWordBool}.
...
Type
  EnumType = (one, two, three, forty = 40,fortyone);
\end{verbatim}
The latter notation is mandatory in mode \var{DELPHI}.
...
As far as the compiler is concerned, the number of elements in an enumeration 
with assigned values is determined by the highest value in the list of values: 
in the above example, the number of elements is 42, but the elements with ordinal 
values 3 to 39 simply do not have a name.
...
\begin{verbatim}
Type
  EnumType = (one, two, forty := 40, thirty := 30);
\end{verbatim}
...
enumeration types with assigned values. Trying to do this anyhow will result in a
compiler error.
\item If the largest element in the list has an ordinal value greater than
256, the enumeration cannot be used to construct a set.
...
Large enum : 4
\end{verbatim}
\item RTTI cannot be generated for assigned enumerated types (C-enumerations).
\end{enumerate}
...
\index{Types!Char}
A character type is also an ordinal type: characters are ordered, can be
counted. There are 2 character types:
\begin{description}
\item[AnsiChar] This is a 1-byte character.
The interpretation of the character depends on the codepage.
\item[WideChar] This is a 2-byte character. The interpretation of the
character depends on the codepage.
\end{description}
Characters can be used in a loop, one can use \var{prev} and \var{succ} on
it, as well as \var{ord}.

For more information on characters, see \sees{charactertypes}.

%
%
%
\subsection{Floating-point types}
\index{Types!Real}\index{Types!Float}
\fpc uses the math coprocessor (or emulation) for all its floating-point
calculations. The \var{Real} native type is processor dependent,
but it is either a \var{Single} or a \var{Double}. Only the IEEE floating point types are
...
Note that not all floating point types are available on all platforms. 
The \var{single} float type is the only one guaranteed to be available on all
platforms that have floating point support (so e.g. AVR does not have it).
The \var{double} type is available on all platforms with coprocessor,
and the \var{extended} type is available on all Intel x86 processors,
...
quotes, as follows: \var{'a'} or \var{'A'} are both character constants.
...
sloppy about the characters it allows after the caret (anything with ASCII
code <128 will work), but in general one should assume only letters.
...
size, and contains one codepoint in UTF-16 encoding.

A Unicode character can be specified by its codepoint
...
the compiler will automatically convert it to a 2-byte UTF-16 codepoint.
...
\fpc supports the \var{String} type as it is defined in \tp: a sequence of single-byte characters with an optional size specification,
these are called shortstrings.
...
The optional size specifier in brackets, e.g. \var{String[128]}, has a twofold effect:
\begin{itemize}
\item The string is a ShortString.
\item The string length is limited to the value given by the specifier.
\end{itemize}

The optional code page specifier in parentheses, e.g. \var{String(1252)}, has a twofold effect:
\begin{itemize}
\item The string is an ansistring with no explicit length limit.
\item The string is associated with the codepage given by the specifier.
\end{itemize}

The size and code page specifiers are optional. A string declaration statement without size and code page specifiers is
interpreted differently depending on the \var{\{\$H\}} switch:
\begin{verbatim}
var
  A : String;
\end{verbatim}
If no size and code page specifier are present, the above declaration can declare
an ansistring or a short string - both are explained in detail in the
following sections.
...
"Short strings" have a declared reserved memory size. This avoids memory
management overhead and can have other advantages in special cases.
It is also used for compatibility with Turbo Pascal,
and they always use the system encoding.

Short strings can be declared in the following ways:
\begin{enumerate}
\item Length is explicitly specified in square brackets.
\item Predefined type \var{ShortString} specified.
\item The specified type is \var{String} and the \var{\{\$H\}} switch is off.
\end{enumerate}
In the last two cases, a maximum length of 255 is assumed,
since the value can only be between 1 and 255, inclusive.

\begin{verbatim}
{$H-} // String is ShortString
var
  S1: String[10];  // can contain a maximum of 10 characters
  S2: ShortString; // can contain up to 255 characters
  S3: String;      // can contain up to 255 characters
\end{verbatim}

\begin{remark}
The value of the \var{\{\$H\}} (\var{\{\$LONGSTRINGS\}}) switch can be
specified explicitly by a directive in the code, taken from compilation
options, from a configuration file, or nothing specified (off by default).
\end{remark}

\begin{remark}
Note that \var{ShortString} is not a special string type in
itself - it is just a pre-declared \var{String[255]}.
Therefore, a maximum length cannot be specified (or overridden) for it:
\begin{verbatim}
var
  S1: String[10];      // correct
  S2: ShortString;     // correct
  S3: ShortString[10]; // error
\end{verbatim}
User programs can also declare similar types:
\begin{verbatim}
type
  TUserName = String[20];
var
...
The maximum string length is part of the type declaration and indicates the size
of the reserved memory. Therefore, it cannot be changed during program execution.

The programmer must take into account the length of the data being assigned,
as if the maximum length is exceeded, the excess portion is simply skipped.
Even enabled range checking will not cause errors.
This must be taken into account for all operations involving short strings
(assignment, concatenation, calling \var{SetLength}, etc).

To get the maximum length of a short string, \var{SizeOf} can be used,
which returns its value plus one.

Don't confuse this with getting the current data length. The actual length
of the string can be obtained with the \var{Length} standard runtime routine:
\begin{verbatim}
var
  S: String[8];
begin
  S:='abcd';
  WriteLn(Length(S)); // 4
  WriteLn(SizeOf(S)); // 9 (8+1)
\end{verbatim}
Note that the \var{SetLength} function also sets the current data length.
A size greater than the maximum cannot be specified.
...
To write more portable code (and independent of different string types),
it is recommended not to rely on this feature, and to use the standard
\var{Length}/\var{SetLength} functions to get/set the length of strings.
...
(so the pointer is \var{Nil}) then the compiler makes sure that the typecast
...
var S: AnsiString;
begin
  L:=length(S); // correct, for AnsiString and ShortString
  L:=ord(S[0]); // error, only works for ShortStrings
\end{verbatim}


The same remark as for \var{Length} must be given:
to set the length of an AnsiString, the \var{SetLength} function must be used.
\begin{verbatim}
var S: AnsiString;
begin
  SetLength(S,4); // corrent, for AnsiString and ShortString
  S[0]:=Char(4);  // error, only works for ShortString
\end{verbatim}
...
Ansistrings can be typecast\index{Typecast} to \var{PChar} or \var{Pointer} types:
...
ansistring is typecast to a pointer, the pointer will be \var{Nil}. If an
empty ansistring is typecast to a \var{PChar}, then the result will be a pointer to a
...
A: "123345123" : 1 -> 1252
\end{verbatim}
During the assignment of \var{A} to \var{B}, the contents of string \var{A}
is converted to the codepage of string \var{B}.
...
\keywordlink{Rawbytestring}
The predefined \var{RawByteString} type is an ansistring string type without codepage information (\var{CP\_NONE}):
\begin{verbatim}
Type
...
Similar to unicodestrings, the compiler transparently converts WideStrings
...
Note that on windows, because a \var{WideString} string is allocated using a special windows function, the
memory layout differs from \var{UnicodeString}. The length for instance is stored in bytes rather than characters.
...
\end{verbatim}
As can be seen, the single quote character itself within the text is represented
by 2 repetitions. Strange characters can be specified by their
...
Note that characters in String start with 1, not 0.
This is important to consider when assigning a pointer:
\begin{verbatim}
...
As already shown, for a ShortString (unlike AnsiString), a terminating null byte
must be explicitly added if you want to use it as a reference for a PChar.
This should also be done when assigning empty strings:
\begin{verbatim}
var
  S: ShortString;
  P: PChar;
...
\begin{remark}
This example also demonstrates that a PChar is not a separate string,
but a reference to an existing one (or even part of one). Therefore,
changing the contents of \var{S} also affects \var{P}.
\end{remark}

The maximum length of a short string must accommodate the entire text,
...
Due to the nature of \var{PChar} strings, many standard string operations
(e.g. concatenation) are unavailable. To work with them correctly, functions
from the \seestrings unit must be used as in the standard C library.

However, some still work. For example, to obtain string length
(taking care of the null character), the standard \var{Length} function
can be used if the \var{PChar} type is explicitly specified (or cast):
\begin{verbatim}
var
  S: String = 'ab'+#0+'cd';
  P: PChar;
...
Also note some peculiarities of pointer arithmetic. The \index{Operators}
...
\var{P - I} & Subtracts \var{I} from the address pointed to by \var{P}. \\
\var{P - Q} & Returns, as an integer, the distance between two addresses \\
 & (or the number of characters between \var{P} and \var{Q}) \\
\hline
\end{FPCltable}

\var{PChar} doesn't necessarily point to the beginning of the string,
but then you must ensure it points to the correct data.
In this case, it will act as a string "truncated" from the beginning:
\begin{verbatim}
var
  S: String = 'abcd'#0;
  P: PChar;
begin
  ...
  WriteLn(P); // cd
\end{verbatim}
Note that "extra" null characters within the string can
even split it into "multiple" \var{PChar} strings:
\begin{verbatim}
var
  S: String = 'ab'+#0+'cd'+#0;
  P: PChar;
...
Structured types can be nested to unlimited levels. There are many possible structured types:
\input{syntax/typestru.syn}
Each of these types will be treated in a separate section or even chapter.
...
\var{SetLength} can also be used for multi-dimensional arrays. The following example will create
a ``rectangular'' array:
\begin{verbatim}
Var
...
begin
  SetLength(A,10,100);
\end{verbatim}
After a call to \var{SetLength}, valid array indexes are 0 to 9 for the
...
array is 1, that is, if two dynamic array variables were pointing to the same
...
\end{verbatim}
The \var{Copy} function will copy six elements of the array to a new array.
...
\index{constants!array}
For typed constants or initialized variables that have as type a static or dynamic array, a constant value must be specified.
A constant array value is specified using round brackets \var{(} and \var{)}, with the elements in between:

\input{syntax/constarr.syn}
...
The constant value for an array cannot be used in an expression, only in a constant or variable declaration. 
...
Object Pascal is a strictly typed language. Two technically distinct types are sometimes considered assignment compatible
...
\end{verbatim}
The opposite typecast is also allowed by the compiler, but must not be
used since the automatic referencing and length determination will not work
correctly. The following program will print '3' for the length of the byte
array, which is of course wrong:
\begin{verbatim}
type
  TIntArr = array of LongInt;
  PLongInt = ^LongInt;
...
\subsubsection{Dynamic array constant expressions}
...
resembles a set expression, in an initialized variable, the same syntax as
...
\subsubsection{Record-typed Constant}
When initializing a variable (or constant) of type record, a special syntax must be used:
for each field in the record, the name must be specified, followed by a colon and an expression for the field value.
The compiler must be able to evaluate the expression at compile time.
The values of the field are separated by a semicolon.

\input{syntax/constrec.syn}

For the \var{TMyRec} record above, the following would initialize all fields:
\begin{verbatim}
var
  R1: TMyRec = (x: 1; y: 2; z: 3);
\end{verbatim}
You can omit some fields from the end, or all of them:
\begin{verbatim}
var
  R2: TMyRec = (x: 1; y: 2);
  R3: TMyRec = (x: 1);
  R4: TMyRec = ();
\end{verbatim}
In this case, the compiler will display a warning:
\begin{verbatim}
Warning: Some fields coming after "y" were not initialized
\end{verbatim}
In \var{Delphi} (and \var{DelphiUnicode}) mode, any fields can be omitted, but the order must be the same:
\begin{verbatim}
var
  R5: TMyRec = (x: 1; z: 3);
  R6: TMyRec = (y: 2; z: 3);
  R7: TMyRec = (z: 3);
\end{verbatim}
The following examples will return an error for any mode:
\begin{verbatim}
var
  R8: TMyRec = (z: 3; y: 2; x: 1); // ERROR
  R9: TMyRec = (z: 3; x: 1);       // ERROR
\end{verbatim}
...
specified is the active one; note that these directives do not accept
exactly the same arguments, see the \progref for more
information).
...
\var{255}. A set can contain at most \var{256} elements.
...
a directive. For more information, see the \progref.

Note that if you use an enumeration with assigned values, if the upper bound
is larger than 256, you cannot construct a set with it.
...
\begin{verbatim}
 BP^[23]
\end{verbatim}
Denotes the 23rd character in the string pointed to by \var{BP}.
...
...
 Inc (p);
\end{verbatim}
will increase \var{P} by 4, because 4 is the size of a longint. If the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Procedural types
\section{Procedural types}\label{se:proceduraltypes}
...
\end{verbatim}
If both pointers must be compared, a typecast to \var{TMethod} must be done, and
both pointers should be compared. \var{TMethod} is defined in the system
unit as follows:
\begin{verbatim}
TMethod = record
  Code : CodePointer;
...
Var
  ...
  B : Integer;
\end{verbatim}
Will actually have the exact same type from the point of view of the compiler
(namely: \var{Integer}). The definition of \var{A} is identical to
\begin{verbatim}
Var
  A : Integer;
\end{verbatim}
...
these two types will be assignment compatible.\index{Type!Compatibility}
...
information in the binary. Since the two types \var{MyInteger} and
...
\item[Variants] They are initialized to the empty value (\var{VarIsEmpty}
will return \var{True}).
\end{description}
...
of the variable as written in the source, using C mangling of the assembler
name (meaning it gets prefixed with a \_ character on some platforms).
...
compiler that the assembler label of this variable should be determined as
the C compiler for the current platform would determine it, additionally it must be made public. i.\,e. it can be
...
\item the eleventh form declares a variable (\var{curterm11}) and
initializes it with a value (1 in the above case).
\item The twelfth form declares a variable (curterm6a), and tells the compiler that it is stored at the absolute address \$00123456 (hexadecimal).
\item The thirteenth form uses an integer expression that resolves to a constant value.
Note that you can take an address of a previously defined variable as base
offset.
\item the fourteenth form shows you can use an assembler name (label) as
address.
\end{enumerate}
...
attempt to export variables with a public name that starts with \var{FPC\_};
the compiler uses some internal system routines with this name.

As for any identifier, variables can be accompagnied by hint directives
(\sees{hintdirectives}):
\begin{verbatim}
var
...
Any assumption that they contain zero or any other default value is erroneous:
...
count to zero, or setting the pointer value of the type to \var{Nil}.
See \sees{managedtypes}
...
$00000000004001D2
\end{verbatim}
Therefore it is {\em Highly recommended} to always initialize variables
before using them.

This can be easily done in the declaration of the variables.
...
must be specified in the form \var{Field: Value}, separated by semicolons,
and surrounded by round brackets.\index{Record!Constant}. You can omit
...
Assigning a character in the string will therefore not work. Assigning \var{S} itself will
...
Note that you cannot have a field in the accessor (read/write) specifiers,
it must be a function or procedure.
...
often as needed.\index{private}\index{public}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Abstract and sealed
\section{Abstract and sealed objects}
\index{Objects, abstract}\index{Objects, sealed}\index{abstract}\index{sealed}

An object can be declared as sealed. In that case, it is not possible to declare a descendent
object. The compiler will return an error if it encounters a declaration of a descendent:
\begin{verbatim}
Type
  TMyObject = object Sealed
    x : integer;
  end;
...
An abstract object is an object that cannot be used directly.
Instead, a descendent object must always be used.
...
is implemented, the fully qualified identifier should be used for the method,
...
It is possible to access the \var{TParent.Doit} from within the
...
For objects, three visibility specifiers exist: \var{private}, \var{protected} and
...
a \var{Protected} section are also accessible to descendent types, even if
they are implemented in other modules. It follows that code outside a
descendant in the module that defines a descendant also will have access to
these protected methods (since the descendant inherits and exposes the protected
methods with the same visibility). \keywordlink{protected}
...
A class can be declared as \var{sealed}. In that case, it is not possible to
...
There are two ways to declare a normal field.
...
Again, there are two ways to define class variables. The first one is equivalent
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Class instantiation
\section{Class instantiation: Creating a class instance}
\label{se:classinstantiation}
\index{Constructor}\keywordlink{constructor}
Class instances must be created using one of their constructors (there can be
...
a class returns a pointer to an initialized instance of the class on the
...
\item If an unhandled exception happens during an the execution of a
constructor, the destructor will be called automatically. In this case,
the variable to which the class instance is to be assigned does not change.
\end{itemize}
\end{remark}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Class destruction
\section{Class destruction : Disposing of a class instance}
...
\begin{remark}
if an unhandled exception happens during an the execution of a constructor,
the destructor will be called automatically. It must be prepared for the fact
that not all fields might have been initialized.
\end{remark}
...
The only differences are the \var{override}, \var{reintroduce}, \var{final} and
\var{message} directives.

\subsection{Implementation}
Method implementations for classes ar no different than for objects:
the implementation is specified using the \var{ClassName.MethodName} identifier:
\begin{verbatim}
Type
   TMyClass  = class
     Procedure DoIt;
...
Note: Because the compiler considers a type alias to be the same type as the aliased type,
type aliases can currently be used when writing the implementation of a method:
\begin{verbatim}
Interface
...
\begin{verbatim}
Warning: An inherited method is hidden by "MyProc;"
\end{verbatim}
...
\index{Class}\index{Methods!Final}
The \var{final} method can be used on a virtual or dynamic (or overridden)
method to indicate it can no longer be overridden.
...
but they still won't be able to read or write instance fields.\index{Property}

\subsection{Class type initialization}
...
\subsection{Covariant return type}
When overriding a function that has a class typed result, it is possible to change
the exact class to a descendant of the result type of the parent method.
...
If no inherited method with the same name exists, the \var{Inherited} will have
no effect in this case. The presence of \var{Inherited} in this form can thus
...
\index{Method!Overloaded}
Similar to function overloading, method overloading simply means that the same 
method is defined more than once in the same class, but each time with a different 
formal parameter list.

The parameter lists must differ at least in one of its element types.
When the compiler encounters a method call, it will look at the method's
parameters to decide which one of the defined methods it should call.
This can be useful when the same method must be defined for different
types.

For example, in the \var{TStrings} class of the \var{Classes} unit, 
the  \var{AddStrings} method has many overloads
\begin{verbatim}
...
\end{verbatim}
When the compiler encounters a call to the \var{AddStrings} method, it will first search
which method it should use. It therefore checks the parameters in a
method call, and looks if there is a method definition which matches the
specified parameter list. If the compiler finds such a method, a call is
inserted to that function. If no such method is found, a compiler error is
generated.

If the \var{overload} keyword is not present, then all overloaded versions
of the method must reside in the same class, i.\,e. the compiler will not 
look for overloaded methods in parent classes if the \var{overload} keyword 
was not specified.
...
Note that class methods cannot be used as read specifiers for regular 
properties, they can only be used for class properties.

A \var{write-specifier} \index{Write} is optional: If there is no \var{write-specifier}, 
the property is read-only. A write specifier is either the name of a field, or
...
As for many identifiers, properties can have hint directives
(\sees{hintdirectives}):
\begin{verbatim}
Property MyProp : Integer read FMyProp; deprecated; 
\end{verbatim}
...
The argument to the \var{stored} specifier should be either a boolean constant,
a boolean field of the class, or a parameterless function which returns a boolean
result. If the argument of the stored specifier evaluates to \var{False},
the property will not be streamed.

\begin{remark}
Absence of a \var{stored} specifier is equivalent to \var{stored True}.
\end{remark}

If the evaluation of \var{Stored} resulted in \var{True}, the default for a
property is considered:

The \var{default} specifier can be specified for ordinal types and sets.
Properties that have as value this default value, will not be written to
the stream by the streaming system, even if \var{Stored} is \var{True}.
...
The \var{nodefault} specifier must be used to indicate
...
\end{verbatim}
\item The \var{nodefault} specifier can be used to override the implicit
\var{default} specifier of string, floating-point and pointer properties.
\item The streaming mechanism described here is the one implemented in the
\file{classes} unit (see \unitsref). Other streaming mechanisms can be
implemented, and they can use the RTTI information in a different way.
\end{enumerate}
\end{remark}

\subsection{Overriding and redeclaring properties}
...
\var{TP1} extends \var{property P} with a default value, \var{TPReadOnly} redeclares \var{property P} as read-only.

\begin{remark}
\var{TP1} should set the default value of \var{P} to \var{1} in its constructor.
\end{remark}
...
The keyword ``inherited'' \index{inherited} can be used to refer to the parent
...
property, but uses the parent''s \var{P} property to store the value.
...
Note that for writeable constants, the same rules apply as for class
...
\index{interface}\keywordlink{interface}
...
\end{verbatim}
\var{IChildInterface} will have two methods: \var{foo} and \var{bar}.
...
Note that when a class declares a child interface, it can be assigned to a variable with the child interface.
...
\end{verbatim}
The reason for this is that although the class actually implements the methods of \var{IParentInterface}, the compiler checks only actually declared interfaces when checking assignment compatibility: all declared interfaces are put in a table and only the contents of this table is checked.

The same check is performed at runtime: the compiler generates a table of all interfaces a class declares, and this table is checked at runtime. That means that although the following will compile if only \var{IChildInterface} is declared:
\begin{verbatim}
...
\index{generic}\index{Type!Generics}\keywordlink{generic}
In Delphi mode, the keyword \var{generic} is not needed. The presence of the template list is sufficient to designate a generic type.

For classes, objects, procedural types and extended records, the generic type declaration should be followed by a type implementation.

It is the same as a normal class implementation with a single exception, namely that any identifier with the same name as one of the template
...
\index{specialize}\index{Type!Specializing}\keywordlink{specialize}
In Delphi mode, the \var{specialize} keyword must not be used.

The generic specialization is a very simple definition and can be used wherever a type definition would be used.

Given the declaration of \var{TList} in the previous section, the following would be a valid type definition:
\begin{verbatim}
Type
...
\end{verbatim}
The \var{specialize} keyword is part of the specialized type, so when using
...
The diagram in \sees{generictypedef} shows that the type template list can have extra specifiers for the types.
...
\end{verbatim}
\section{Generic procedures and functions}
\label{se:genericprocs}
\index{Procedures!Generic}
\index{Functions!Generic}
As can be seen in the syntax diagrams for generic procedures and functions,
it is possible to define generic procedures and functions or generic
methods. Where in a generic class type all methods of the class can contain the
template, it is possible to have a non-generic class with a generic method:
\begin{verbatim}
uses typinfo;
...
\end{verbatim}
But this code is not compilable with Delphi. In delphi, the closest thing to
this feature is to use a record or class with generic class methods:
\begin{verbatim}
uses typinfo;
...
\index{Type!Compatibility}
Whenever a generic class is specialized with a different set of template
types, this results in a new, distinct type.

The types are assignment compatible if the same template types are used. 

If the same types are used, and compiler can reuse a previously generated
generic, it will reuse this previously generated generic, but this is
not guaranteed to be possible: you may still have 2 separate generic types,
but they will be assignment compatible.
...
Beside the template type, all other types used in the generic declaration must be
known when the declaration is parsed.
...
\end{verbatim}
Will not compile, unless the addition of two \var{TComplex} types is defined.
...
them, and properties. They can also have local constant and type definitions. 

Like objects, when defined as a variable, the extended records are
...
As of version 3.2.4 of Free Pascal, extended records can have constructors, on the
condition that they have parameters. A parameterless constructor is not allowed.


If no visibility is specified for a record, it is \var{public} by default,
which is in line with 'classic' records in which all fields are always visible.
...
\section{Record operators}
Operator overloading is treated in detail in the chapter on operator overloading \seec{operatoroverloading}.
...
records. This syntax is available in both \var{ObjFPC} and \var{Delphi} mode.

In \var{objfpc} mode, the operators must be defined with their symbolic
...
Note that they have the \var{class} keyword prefixed, and have the record type
name prepended in the implementation.


As indicated above, in \var{ObjFPC} mode, the operators must be indicated with their symbol names.
By contrast, in \var{Delphi} mode, the names of the operators can also be used, similar to the syntax in Delphi:
\begin{verbatim}
{$mode delphi}
Type
  TComplex = record
...
\begin{FPCltable}{cl}{Operator names}{operatornames}
Symbol & Name \\ \hline
\var{+} & add or positive \\
...
\var{div} & intdivide \\
\var{mod} & modulus \\
\var{and} & bitwiseand or logicaland \\
\var{or} & bitwiseor or logicalor \\
\var{xor} & bitwisexor \\
\var{not} & logicalnot \\
\end{FPCltable}

For example, the power operator (**) can be used in \var{Delphi} mode using its
symbolic name:
\begin{verbatim}
{$mode delphi}
{$modeswitch advancedrecords}
...
operator overloading \seec{operatoroverloading}.
...
\var{helper}, it just cannot be the first one, unless it is escaped with an
...
the programmer. It is therefore clear what the \var{Self} parameter means and when it is valid.
...
and can therefore be used as event handlers, this can lead to surprising situations:
...
\end{verbatim}
will evaluate to \var{True}, because it is equivalent to
\begin{verbatim}
  True or (True and False)
\end{verbatim}
\end{remark}
...
The \var{variable-reference} must be a procedural type variable reference.
A method designator can only be used inside the method of an object. 
A qualified method designator can be used outside object methods too.
...
\index{Constructor}
When a dynamic array constant must be entered in an expression, an array constructor can be given.
An array constructor is a comma separated list of expressions enclosed in square brackets.
The result type of each expression in the list must be compatible to the array element type. 

\input{syntax/arrayconst.syn}
The empty array is denoted by \var{[]}, and it can be assigned to any type of
dynamic array.
...
[ 2, 3*2, 6*2, 9*2 ]
\end{verbatim}
The second and third expression are potentially sets. 
The compiler must be able to determine whether the result needs to be a set or an array.
...
\begin{remark}
Note that if you typecast a value (or variable), the compiler will assume that the
result of the typecast is a valid value for the type you typecasted it to. 
In particular, this means no range checks will be inserted, even when range checks are
enabled.
\end{remark}
...
\end{verbatim}
But not all typecasts can be assigned to. The above will work, but e.g. a
typecast to single cannot be assigned to.
...
\begin{itemize}
\item No overflow checking is done on logical operations. Range errors may
of course occur when the result of a logical operation is assigned to a
variable.
\item Bit shift operations that shift bits with more positions than the 
\var{(bit size of operand)-1} are undefined. The result of a negative 
shift is also undefined.
\end{itemize}
...
\index{Operators!Dynamic arrays}
...
\var{arrayoperators} in objfpc mode:
\begin{verbatim}
{$mode objfpc}
{$modeswitch arrayoperators}
\end{verbatim}
When enabled, its action is similar to
the concatenation for strings: to concatenate the contents of the two arrays it acts on.
...
The operators are listed in \seet{setoperators}.
\keywordlink{in}
\begin{FPCltable}{ll}{Set operators}{setoperators}
Operation & Action \\ \hline
\var{+}  & Union \\
...
\var{in}  & check whether an element is in a set\\
\end{FPCltable}
In addition to the above operators, elements can be added or removed from the set with the \var{Include} or
\var{Exclude} procedures:
\begin{description} 
\item[include] includes an element in the set.
\item[exclude] excludes an element from the set.
\end{description} 

The set type of the operands or arguments must be the same, or an error will be
...
are only two class operators, as can be seen in \seet{classoperators}.
...
\var{a} (only for floats, not integers). \\ \hline
\end{FPCltable}

For C-like assignments, the \var{\$COPERATORS} directive must be enabled
or the \var{-Sc} command-line switch must be specified when compiling.
These constructions are just for typing convenience and generate the same code.
...
The case statements can be compound statements (i.\,e. a \var{Begin..End}
block), but for the else case multiple statements are allowed (as can be
seen in the syntax diagram). You may enclose them in an extra \var{Begin..End}
block, but it is not necessary.

Note that the semicolon in front of the \var{else} clause is optional.
This may lead to confusion if the last label contains an if statement.

In the following example, when \var{a} equals 3, nothing will be executed:
\begin{verbatim}
case a of
1 : 
  DoSomething;
...
\end{verbatim}
But in the following example, when \var{a} equals 3, \var{DoSomethingElse}
will be executed.
\begin{verbatim}
case a of
1 : 
...
\end{verbatim}
When in doubt or to avoid confusion, you can enclose the last statement in an extra \var{Begin..End}
block.
...
\end{verbatim}
The \var{else} clause can contain multiple statements:
\begin{verbatim}
Case Number of
 1..10   : WriteLn ('Small number');
 11..100 : WriteLn ('Normal, medium number');
...
\begin{remark}
Note that in Delphi mode, the use of strings as case labels is not allowed.
\end{remark}
...
\item
In nested \var{If.. then .. else} constructs, some ambiguity may arise as
...
\item the loop can be exited using the \var{break} system routine.
\item the loop can be forced to the next iteration (if any) using the
\var{continue} system routine.
\item The value of the loop variable is undefined after a loop has completed or if a loop is not executed at all.
However, if the loop was terminated prematurely with an exception or a \var{break} or \var{goto} statement,
the loop variable retains the value it had when the loop was exited.
\item if the loop variable is the \var{Result} variable, and the loop is
ended with \var{exit}, the last value before exit will be retained.
...
\item The compiler does not expliticly forbid jumping with a \var{goto} statement into a for loop block,
but doing so will result in unpredictable behaviour.
...
Inside the loop statement the \var{Break} and
\var{Continue} system routines can be used to jump to the end or just
after the end of the \var{For} block statement: in the case of
\var{Continue}, this means that the for loop condition is again evaluated.
In the case of nested loops, only the innermost loop is considered.
Note that \var{Break} and \var{Continue} are not reserved words and therefore can be overloaded.

Note that for enumeration values with assigned values, the non-named values
...
\end{verbatim}
Will print 101 values: 0 to 100.

\subsection{The \var{For..in..do} statement}
\label{se:forin}\index{Statements!For}\index{Statements!Loop}\index{For}\index{For!in}
...
enumeration type. An enumeration type with assigned values cannot be used. 
...
\end{verbatim}
A \var{string} type is equivalent to an \var{array of char}, and therefore
...
\begin{remark}
\begin{itemize}
\item A loop can be exited prematurely using the \var{break} system routine.
\item The next iteration of a loop can be forced using the \var{continue} system routine.
\item Like the \var{for..to} loop, it is not allowed to change (i.\,e. assign a value to) the value of a
...
In the case of \var{Continue}, this has the effect of immediatly checking the
loop condition, bypassing all code between \var{Continue} and the \var{until}
keyword. In the case of nested loops, only the innermost loop is considered.
Note that \var{Break} and \var{Continue} are not reserved words and
...
In the while statement the \var{Break} and \var{Continue} system routines can be used to 
jump to the end or just after the end of the \var{While} block. 
In the case of \var{Continue}, this has the effect of immediatly checking the loop condition,
bypassing all code between \var{Continue} and the end of the loop block.
In the case of nested loops, only the innermost loop is considered.
Note that \var{Break} and \var{Continue} are not reserved words and
therefore can be overloaded.
...
\end{remark}
\begin{remark}
There is a subtle difference between Delphi and Free Pascal when resolving identifiers in a \var{With}  statement:
Free Pascal will always search first in the scope of the expression used in the \var{With} statement, 
and only then will it search in the local scope. Delphi will first search in the local scope and then only in the with scope:
\begin{verbatim}
Type
  TField = Record
    Procedure Updating;
...
Note that procedures can be generic, see \sees{genericprocs} for more
details.
...
As with most identifiers, a procedure can have hint directives
(\sees{hintdirectives}):
\begin{verbatim}
...
Note that functions can be generic, see \sees{genericprocs} for more
details.

Just as procedures, functions can have hint directives
(\sees{hintdirectives}):
\begin{verbatim}
...
%
\subsection{Constant
parameters}\index{Parameters!Constant}\keywordlink{const}\keywordlink{constref}
...
Specifying a parameter as constant using \var{const} or \var{constref} is giving the compiler a hint that the
contents of the parameter will not be changed by the called routine.
\var{constref} additionally tells the compiler the value should be passed by
reference (just as a \var{var} parameter).

This allows the compiler to perform optimizations which it could not do otherwise,
...
\begin{remark}
Contrary to earlier versions of Delphi, no assumptions should be made about
how \var{const} parameters are passed to the underlying routine.
In particular, the assumption that parameters with large size are passed by reference is not correct.
For this the \var{constref} parameter type should be used, which is available as of
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The UNIV parameter type
\subsection{Macpas: UNIV typed parameters}
\index{Parameters!UNIV type}\index{UNIV}
In Macpas mode, you can specify a \var{UNIV} (universal) parameter. 
This signifies that any parameter with the exact same byte size as the declared parameter can be passed as the value of the parameter:
Given the following procedure declaration:
\begin{verbatim}
procedure HDispose( var theHandle: univ Handle); 
\end{verbatim}
You can pass value of any type that has the same size as \var{Handle}. 
...
\end{verbatim}
You can call \var{Find} as follows
\begin{verbatim}
 Result:=Find(@theCmdID,1,kCmdCount,IsSmallerThan, IsEqualTo,aIndex):
\end{verbatim}
Where the two functions \var{IsSmallerThan} and \var{IsEqualTo} are defined as:
\begin{verbatim}
function IsSmallerThan(theIndex : Int32; aPtr : PtrInt): boolean;
function IsEqualTo(theIndex : Int32; aPtr  : PtrInt): boolean
\end{verbatim}
Because \var{PtrInt} has the same size as \var{Pointer}, this will compile.
...
\end{verbatim}
Therefore, inside the procedure body, the \var{array of const} argument is equivalent to
...
\begin{remark}
Note that there is no support for \var{DWord} (or cardinal) arguments in \var{array of const}.
They are converted to \var{vtInteger}/\var{vtLongint}.
This is for Delphi compatibility, and the compiler will ignore any resulting range checks
when in mode Delphi.
\end{remark}

\subsection{Untyped parameters}
\index{Parameters!Untyped}
...
Inside a routine with an untyped parameter, if the untyped parameter is used in an
...
\subsection{Ellipsis parameter}
\index{Parameters!ellipsis}
In macpas mode you can have an ellipsis parameter. 
The ellipsis (\var{\ldots}) parameter can appear only once, and must be the last parameter in the parameter
list. Additionally it can only appear after at least one other parameter.
It has the same effect as the \var{varargs} modifier: it tells the compiler that a variable number of arguments is possible.
...
\index{Parameters!Reference counted}
...
For example, in a hypothetical RTL, the  \var{Random} procedure could be defined as:
\begin{verbatim}
Random(Max : Longint);
Random(Max : Double);
Random(Max,Ofsset : Longint);
\end{verbatim}
When the compiler encounters a call to the \var{Random} function, it will first search
...
the case with \var{Inc()} and \var{Dec()}
\end{remark}
...
Sometimes, the external symbol may not be there. In that case, you can use
the \var{weakexternal} keyword instead of the \var{external} keyword. For
example:
\begin{verbatim}
function _test: cardinal; weakexternal libname name '_TEST';
\end{verbatim}
If the function is not there at runtime, the function reference will be
\var{Nil}. This means that before using such a function, you need to test
...
The call modifiers can be found in the section on procedural types (\sees{proceduraltypes}).
...
\begin{remark}
In macpas mode, the \var{cdecl} modifier can be abbreviated to \var{c}.
\end{remark}  

\subsection{compilerproc}
\label{se:compilerproc}\index{compilerproc}\index{Modifiers!compilerproc}\keywordlink{compilerproc}
The \var{compilerproc} modifier is used internally by the compiler. It marks
helper routines which you should never call directly, they are used by the
compiler to implement routines for type conversions, concatenate string etc.

As a user, you should never use this modifier. You should also never call
them directly, the compiler will issue calls to such procedures.

\subsection{cppdecl}
\label{se:cppdecl}\index{cppdecl}\index{Modifiers!cppdecl}\keywordlink{cppdecl}
The \var{cppdecl} modifier can be used to declare a function that uses a C++
type calling convention. This must be used when accessing functions residing in
an object file generated by standard C++ compilers, but must also be used for
Pascal functions that are to be used as callbacks for C++ libraries.
...
\subsection{hardfloat}\keywordlink{hardfloat}
\index{hardfloat}\index{Modifiers!hardfloat}
The \var{hardfloat} modifier is used to indicate that the compiler must
...
\item You can specify \var{inline} in the interface of a unit or in the
implementation, the result is the same.
\item If you declare an inline function in the interface part of the unit,
then the implementation must appear before the first use of this function, or
the compiler will not inline it.
\end{enumerate}
\end{remark}
...
\subsection{MS\_ABI\_Default}\keywordlink{MS\_ABI\_Default}
\index{MS\_ABI\_Default}\index{Modifiers!MS\_ABI\_Default}
The \var{MS\_ABI\_Default} modifier is used to indicate that the compiler must
...
\subsection{MS\_ABI\_CDecl}\keywordlink{MS\_ABI\_CDecl}
\index{MS\_ABI\_CDecl}\index{Modifiers!MS\_ABI\_CDecl}
The \var{MS\_ABI\_CDecl} modifier is used to indicate that the compiler must
...
\subsection{MWPascal}\keywordlink{MWPascal}
\index{MWPascal}\index{Modifiers!MWPascal}
The \var{MWPascal} modifier is used to indicate that the compiler must
...
There are two cases where the \var{overload} modifier is mandatory:
...
Secondly, if it concerns methods part of a class, if the overloaded methods
are not in the same class. Without the \var{overload} keyword, the compiler 
will not look for overloaded methods in parent classes.

The following example illustrates the first case. Take the first unit:
\begin{verbatim}
unit ua;
...
The \var{register} keyword is used for compatibility with Delphi.
...
\subsection{SYSV\_ABI\_Default}\keywordlink{SYSV\_ABI\_Default}
\index{SYSV\_ABI\_Default}\index{Modifiers!SYSV\_ABI\_Default}
The \var{SYS\_ABI\_Default} modifier is used to indicate that the compiler must
...
\subsection{SYSV\_ABI\_CDecl}\keywordlink{SYSV\_ABI\_CDecl}
\index{SYSV\_ABI\_CDecl}\index{Modifiers!SYSV\_ABI\_CDecl}
The \var{SYSV\_ABI\_CDecl} modifier is used to indicate that the compiler must
use the x86-64-specific calling convention that conforms to the System V AMD64
ABI, with the exception that 'array of const' are interpreted as cdecl varargs parameters,
instead of the regular pascal array of const.

\subsection{VectorCall}\keywordlink{VectorCall}
\index{VectorCall}\index{Modifiers!VectorCall}
The \var{VectorCall} modifier is used to indicate that the compiler must
use vector call calling convention. This is a variant of the
\var{MS\_ABI\_Default} calling convention that passes certain
...
\subsection{winapi}
\index{winapi}\index{Modifiers!winapi}\keywordlink{winapi}
...
depending on the OS architecture: \var{stdcall} on all windows platforms and
\var{cdecl} on all other
platforms.
...
In addition, operator overloading can be used for some special actions
that are strictly speaking not operators in the mathematical sense of the word.
This is true for the procedures \var{Inc} and \var{Dec} (which are usable as
...
operator(s) must be \var{Boolean}.
...
\begin{itemize}
\item This way of declaring operators is only supported in \var{ObjFPC} and
\var{FPC} modes.
\item It is not possible to define operators on classes.
\item The + and - operators cannot be defined on enumerated types.
\item When the \var{ArrayOperators} modeswitch is in effect:
\begin{verbatim}
{$modeSwitch ArrayOperators}
\end{verbatim}
...
\begin{remark}
When compiling in \var{Objfpc} mode, the result identifier may be dropped.
The result can then be accessed through the standard \var{Result} symbol.
...
Logical operators \var{and}, \var{or}, \var{xor} and \var{not} can be overloaded.
These operators are normally used on simple types in two different ways:
\begin{enumerate}
\item As boolean operators, in which case the result is a boolean (except for \var{not})
...
The \var{Inc} and \var{Dec} procedures are~-- for optimization purposes~-- compiler intrinsics. They can be
...
The program header is not required. If present, it will be used by the compiler
to name the program. If absent, the source filename will be used. 
...
\end{verbatim}
\file{unita} is searched in the parent directory of the current working
directory of the compiler. You can add a \var{\{\$UNITPATH ..\}} directive
to make sure that the unit will be found no matter where the compiler's
...
A note on the program identifier: 
\begin{itemize}
\item It can be used to refer to a variable, in order to distinguish between
...
\end{verbatim}
\item When linking in resources using a wildcard, the filename of the program is
used to search the resource file. The name will not be used as the name of the 
resource file. So in a file called \var{my.pas} the following code
\begin{verbatim}
program mytest;
...
The unit header can be accompagnied by hint directives
(\sees{hintdirectives}):
\begin{verbatim}
unit old deprecated;
\end{verbatim}
...
With one exception: in macpas mode, the \var{implementation} 
keyword may be omitted if the implementation is empty..
.
...
\end{verbatim}
Because \file{UnitB} uses \file{UnitA} only in its implementation section.
...
Note that when you use 2 units declare the same symbol, the definition of the symbol 
in the last unit in the uses clause is used, i.e. the compiler searches in used units from last to first.
Thus, in the following set of units:
\begin{verbatim}
unit unitA;
...
\end{verbatim}
\var{MyVar} will be of type \var{integer}.

The compiler will always search in the current unit first before searching in the uses clauses.
Thus, in the following units, the type of \var{MyVar} is again \var{integer}.
\begin{verbatim}
unit unitA;
...
\end{verbatim}
The compiler will first search for \var{MyType} in the current unit, and if it is not found there, it will look in the uses clauses.
...
\input{syntax/exception.syn}
\begin{description}
\item [Raise statement] To raise an exception. This is usually done to signal an
...
executable.
\item [Try ... Except statement] This is a structured statement. This block of statements serve to catch exceptions
raised within the scope of the block, and to provide exception-recovery
code.\index{Exceptions!Catching}
\item [Try ... Finally statement] Is also a structured statement. This block of statements force code to be
...
\end{verbatim}
The class \var{Exception} (\sees{exceptclasses}) is defined in the
\file{SysUtils} unit of the RTL (see \unitsref).
...
\chapter{Pascal grammar}

Below is a formal grammar of the pascal language in EBNF notation.
It is not 100\% complete, some parts such as assembler or objective-c have
not been specified.

\begin{grammar}
\renewcommand{\syntleft}{$\langle$\normalfont\itshape}
\renewcommand{\syntright}{$\rangle$}
\renewcommand{\litleft}{\"}
