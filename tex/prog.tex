...
Some directives can only take a boolean value, which means a '+' to switch them on,
or a '-' to switch them off. No space should be present between the boolean
switch and the + or - sign. These directives are also known as switches.
...
may be replaced by the \var{ON} or \var{OFF} keywords. In that case, there
must be a space between the directive name and the \var{ON}/\var{OFF}.
...
Note that anything after the ON/OFF or +/- or a value required by the
directive, is ignored. i.e. the compiler parses the directive as far as it
needs to in order to evaluate it, but the rest is skipped.
...
Note that the configuration file created by the installer can change some of
the compiler differences mentioned here.
...
assembler.
\item [default\ ] Tells the compiler to use the default assembler style as
specified on the command-line, or, if it was not specified on the
command-line, the default for the current platform. This can be used to reset
the value to the default if it was changed.
\end{description}
...
\begin{remark}
The calling convention of a procedural type is also changed by this directive.
\end{remark}
...
Note that this only works for pointers managed by FPC. If you receive
and use pointers from external libraries, the checks will fail.
...
When the \var{\$COPERATORS} directive or the \var{-Si} options are enabled,
some assignments in the style of a C language available (disabled by default).
They allow shorthand notation for some constructs:
\begin{verbatim}
var
  N: Double;
begin
...
This form is no different from the regular one and generates the same code
(and has the same requirements, for example, regarding the operand types).

Note that division is the Pascal \var{/} operator, not \var{div}, so it's only
available for float numbers. For integers, the full form (\var{N := N div 5})
must be explicitly used.

The expression to the right of the operator is evaluated first, then the operator itself.
For example, the result of the following expression will be \var{6}, not \var{4}:
\begin{verbatim}
N:=3;
N*=1+1; // same as N:=N*(1+1)
\end{verbatim}
...
\subsection{\var{\protect\$EXCESSPRECISION}: use excess precision with float}

This directive tells the compiler to use maximum precision when evaluating
floating point expression, i.e. use the maximum possible precision, even if
the members in the calculation have less than maximum precision. 
It accepts a boolean value (\var{ON}/\var{OFF} or +/-). 
...
Note that the compiler will only consider valid expressions. As soon as
something invalid in the expression is encountered, the rest is considered
comment. In the following, both conditions are checked:
\begin{verbatim}
{$if defined(define1) or defined(define2)}
\end{verbatim}
But in the below, only \var{define1} is checked, the rest is considered a
comment:
\begin{verbatim}
...
\subsection{\var{\protect\$IFEND} : End conditional compilation}
This directive can be used as an alternative to the \var{\$ENDIF} directive. 
...
\begin{remark}
Note that the time arguments are the times when the directive is processed.
Thus it is possible that different time stamps appear in the strings even if
they are in the same unit but on different locations in the file.
\end{remark}
...
\begin{remark}
The compiler internally uses library names \var{libimp*.*}; as a side
effect, you cannot use the LINKLIB statement to link to a library with such
a name.
\end{remark}

\subsection{\var{\protect\$M} or \var{\protect\$TYPEINFO} : Generate type info}
\label{se:dirtypeinfo}
...
Note that this directive has the same abbreviation as the global \var{\$MEMORY}
directive: \var{\$M} (\sees{dirmemory}).
...
Not to be confused with the \var{\$UNDEF} directive.
...
In case of typecasts, an implicit type check will be generated in the ON state, 
the type check will be omitted in the OFF state.

This switch is also activated by the \var{-CR} command line option,
so you can undo the effects of \var{-CR} on objects with  
\begin{verbatim}
{$OBJECTCHECKS OFF}
\end{verbatim}
...
\item[OFF] Switches off all kinds of optimizations.
...
\item[STRENGTH] Only used when DFA is also specified: try to replace a loop over an array index with a pointer that runs over the elements.
\item[AUTOINLINE] Try to automatically inline procedures where appropriate.
\item[LEVEL1] Enable some level 1 optimizations, dependent on the CPU. 
Note that this is not the same as \var{-O1}, it is a separate flag.
\item[LEVEL2] Enable some level 2 optimizations, dependent on the CPU. 
Note that this is not the same as \var{-O2}, it is a separate flag.
\item[LEVEL3] Enable some level 3 optimizations, dependent on the CPU.
Note that this is not the same as \var{-O3}, it is a separate flag.
\item[LEVEL4] Enable some level 4 optimizations, dependent on the CPU.  
Note that this is not the same as \var{-O4}, it is a separate flag.
\item[SCHEDULE] ARM CPU only: perform instruction scheduling. Enabled by default at -O3 and higher. 
\item[USEEBP] i8086 and i386 CPUs only: use the (E)BP register for register variables when
it is not used to hold the frame pointer. Enabled by default when using
level 4 optimizations.
\item[USERBP] x86-64 CPU only: use the RBP register for register variables when it
is not used to hold the frame pointer. Enabled by default when using
level 4 optimizations.
\item[DEADVALUES] Allow removing expressions whose result is not used, even when this can
change program behaviour. For example, range check errors can disappear when
indexing an array and not doing anything with the loaded value, or
access violations due to invalid pointer dereferences can disappear.\\
{\em Note:} it does not (and must not) remove expressions that have explicit
...
Note that the optimizations are not saved/restored by the \var{\$PUSH} and
\var{\$POP} directives.
...
\var{\{\$Z4\}}. The \var{\{\$Z\}} form takes a +/- argument, 
where \var{+} is equivalent to \var{\{\$Z4\}} and \var{-} is 
...
It is important to note that the number of elements is determined by the
highest value of the enumeration. If the enumeration has assigned values, then
the "number of elements" is actually the highest value in the enumeration+1.
\begin{verbatim}
program Project1;
{$PACKENUM 2}
type
...
\end{verbatim}
will print 4 because the number of elements is \var{\$FFFF+2}
...
More information on this and an example program can be found in the \refref,
in the section about record types.
...
switches (so not all directives) with the last values that were stored 
on the settings stack. The settings are then deleted from the stack.
...
switches except optimization settings on the settings stack. 
Up to 20 sets of settings can be stored on the stack.

Some verbosity directives (\var{\$Warn} etc.) as well as the \var{\$PACKENUM},
\var{\$PACKRECORDS} and \var{\$SETALLOC} are also saved.
...
\end{remark}
\begin{remark}
The \var{-CR} and \var{-Cr} command-line options will activate the range
checks globally. They can be switched off locally with the \var{OFF} state
of the directive.
\end{remark}
...
In its default state (\var{OFF}) the enumeration values are additionally inserted 
...
If the directive is set to \var{ON} then the enumeration values are {\em
...
\end{verbatim}
i.e. in the \var{ON} state of the directive, the value must always be prefixed with the type name.
...
pointer types. This directive is equivalent to the \var{-Sy} command-line
option.
...
Note that the abbreviation of this directive is the same as the local
directive \var{\$M} for enabling Run-time Type information (\sees{dirtypeinfo}).
...
\item[ARRAYOPERATORS] Use Delphi compatible array operators instead of custom ones ("+").
...
\item[FINALFIELDS] Allows declaring fields as "final", which means they must be initialised in the (class) constructor and are constant from then on (same as final fields in Java).
...
delphiunicode mode: \var{\{\$mode delphiunicode\}}.  
...
To have a smartlinked binary, 2 command-line options are needed:
\begin{description}
\item[-CX] create object files that can be smartlinked (or use the directive above)
\item[-XX]: perform smartlinking during the linking phase. This requires that
the units were compiled with \var{-CX}
\end{description}

It should be noted that on some targets, \var{-CX} is a no-op and \var{-XX} 
works even when \var{-CX} was not specified (e.g. Linux), or both \var{-CX} and \var{-XX} 
are no-ops and smart linking is always enabled (as on Windows).

\subsection{\var{\protect\$SYSCALL} : Select system calling convention on Amiga/MorphOS}
...
path. \var{XXX} can contain one or more paths, separated by semicolons or
colons. No quotation marks must be present, even if there are spaces in the paths.
...
which the directive appears. The compiler will look for the file \file{strings.ppu}
in both these directories, and will link the first found file in the
program.
...
are relative to the current unit). To add search paths for "all units",
need to use the command line switch \var{-Fu}.

The \var{uses} clause construct \var{uses a in 'sub/a.pp'} is relative to the 
...
occurrence of the macro in the pascal source will be replaced verbatim with the value
of the macro (much like the macro support in the C preprocessor). There is
no support for function arguments; only verbatim replacement of the macro
name with its value is supported.

If macro support is required, the \var{-Sm} command line switch must be used to
...
{$IF DEFINED MySym}
\end{verbatim}
Note that you cannot use a macro that resolves to a symbol, the symbol name
must be used.
...
constant. Note that you cannot use a macro that resolves to a symbol, the symbol name
must be used.
...
\ref{ch:AppA}, on page \pageref{ch:AppA}.
...
By default, the calling mechanism the compiler uses on I386 is \var{register}, 
...
\refref, section on modifiers for additional calling conventions.
\end{remark}
...
declaration, but is subject to C name mangling (it can e.g. be prefixed with
a \_ on some platforms): this means the name is a case sensitive name, so you must be careful.
\end{enumerate}
...
This field is stored first, and all fields in the class are stored, not
necessarily in the order they are declared: the compiler may rearrange the
fields to better pack them.
...
32-bit & 64-bit & \\ \hline 
+0 & +0 & Pointer to VMT. \\
+4 & +8 & Data. All fields in the order they've been arranged by the compiler. \\
\dots & \\
\hline
\end{FPCltable}
...
a summary will be sent to standard error.
...
\item[GetFPCHeapStatus] should return a \var{TFPCHeapStatus} record with the
status of the memory manager. This record should be filled with
...
extension \file{.rsj}. The format of this file is a JSON file (in codepage UTF8), 
...
called \var{strings}. The array contains an element (a JSON object) for every resource string in the
unit. This JSON object contains the following properties:
\begin{enumerate}
\item \var{hash}: a has value of the string, for fast lookup.
\item \var{name}: the fully qualified name of the resource string, in all lowercase letters. 
The name is composed of the unit name, a dot and the resource string
identifier.
\item \var{sourcebytes}: The sourcebytes are the bytes that compose the string as they
were read from the source file.
\item \var{value} the value of the resource string, in UTF8.
\end{enumerate}
...
Procedure SetUnitResourceStrings (UnitName: string;
                               ...
                               arg: pointer);
\end{verbatim}
Other functions exist, for convenience only:
\begin{verbatim}
...
of the callback. 
The \var{arg} pointer is passed to the callback (this can be used to pass e.g. an object
instance).
\item[SetUnitResourceStrings] Is identical in behaviour to
\var{SetResourceStrings}, but only calls the callback for the resource
strings of unit \var{UnitName}.
...
\end{enumerate}
An example program \file{restest.pp} exists in the FCL-base sources, in the \file{fcl-base/examples}
directory.
\section{Caveat}
...
\item[TryEnterCriticalSection] When this call returns, the return value is zero if another 
thread owns the critical section, or nonzero if the current thread already owns or successfully obtained the critical section. 
In the latter case, the thread is the only thread running the code between the \var{tryEnterCriticalSection} 
call and the following \var{LeaveCriticalsection} call.
...
or \var{Succ} functions (in general any mathematical function that is
handled internally by the compiler) with constant parameters generates no
...
Using the \var{in} operator is always more efficient than using the
...
\file{libsubs.dylib} on MacOS, \file{libsubs.so} on other \unix systems, 
or \file{subs.dll} on \windows or \ostwo.
...
\var{\{\$linklib \}} directive, as in the following example:
\FPCexample{psubsm}
...
\begin{remark}This means that the asterisk may only be used after a \var{unit},
...
\item Procedure or Function can have up to 255 parameters.
...
this mode, the compiler complies with the requirements of level 0  
...
To install the newly compiled compiler, the \var{install} target can be
used:
\begin{verbatim}
make install
\end{verbatim}
...
The makefile supports compiling in parallel. For this, you can use the  \var{-j N}
make option, to start \var{N} processes. 

For the packages and utils, the compilation is governed by \var{fpmake}, and make 
just starts \var{fpmake}. 

The \var{fpmake} process can also compile in parallel, but this needs a separate option 
\var{-T N}. You need to specify this option in the
\var{FPMAKEOPT} variable:
\begin{verbatim}
...
information.  You can force debug information using the \var{OPT} variable:
\begin{verbatim}
make all OPT=-gl
\end{verbatim}
If you want heaptrc for checking for memory leaks, then add the \var{-glh} option:
\begin{verbatim}
make all OPT=-glh
\end{verbatim}

\subsection{Various other options}
...
compiler command-line options to \var{OPT}:
\begin{description}
\item[LINKSMART=1] adds \var{-XX} to the compiler command-line options.
\item[CREATESMART=1] adds \var{-CX} to the compiler command-line options.
\item[DEBUG=1] adds \var{-gl} and \var{-dDEBUG} to the compiler command-line options.
\item[RELEASE=1] adds \var{-O1} or \var{-O2} depending on the CPU, and
\var{-Ur} and \var{-DRELAASE} to the command-line.
\item[STRIP=1] adds \var{-Xs} to the command-line.
\item[OPTIMIZE=1] adds \var{-O2} to the command-line.
\item[VERBOSE=1] adds \var{-vwni} to the command-line.
\end{description}
...
\begin{verbatim}
ppc386 -Tlinux  -Fusystems -Fux86 -Fii386/ -Fui386 \
...
\begin{verbatim}
ppc386 -Fusystems -Fux86 -Fii386 -Fui386 -di386 -dGDB -Sg pp.pas
\end{verbatim}
...
FPC\_ISO & \fpc is in ISO 7185 Pascal mode, either using compiler switch -Miso or \\ 
& using the \var{\$MODE ISO} directive. \\
FPC\_EXTENDEDPASCAL & \fpc is in ISO 10206 Pascal mode, either using compiler switch \\ 
& -Mextendedpascal or using the \var{\$MODE EXTENDEDPASCAL} directive. \\
\hline
\end{FPCltable}
...
CPUSPARC32 & \fpc target is a 32-bit SPARC v7 or compatible.\\
CPUSPARC64 & \fpc target is a 64-bit SPARC v7 or compatible.\\
CPUSPARCGEN & \fpc target is a SPARC v7 or compatible.\\
...
CPUAARCH64 & \fpc target is an ARM 64-bit processor.\\
CPUAVR & \fpc target is an AVR 16-bit processor.\\
CPU16 & \fpc target is a 16-bit CPU.\\
CPU32 & \fpc target is a 32-bit CPU.\\
CPU64 & \fpc target is a 64-bit CPU.\\ 
CPUI8086 & indicates a 16-bit x86 target (i8086) \\ \hline
\end{FPCltable}
...
FPUSSE2 & SSE2 instructions on Intel I386 and higher. \\
FPUSSE3 & SSE3 instructions on Intel I386 and higher, AMD64. \\
FPUSSE41 & SSE41 instructions on Intel I386 and higher, AMD64. \\
FPUSSE42 & SSE42 instructions on Intel I386 and higher, AMD64. \\
...
FPC\_HAS\_CEXTENDED & Support for AT\&T assembler \var{.tcfloat} type\\
FPC\_HAS\_RESSTRINITS & Initialization of resource strings \\
FPC\_HAS\_INTERNAL\_ROX & Internal ROX command \\
FPC\_HAS\_LWSYNC & Support for lightweight sync of load/store operations at assembler level. \\
...
FPC\_HAS\_FEATURE\_SUPPORT & FPC has support for the various HAS\_FEATURE\_NNN macros \\
